...
data_list <- standata(new_stan_results)
data_list$zip_code_distance <- zip_code_distance 
data_list$l = 5 
data_list$sigma = 1 
...
mod <- cmdstan_model("code/stan/default_brms_stan_code_gp.stan")
...
for (i in 1:100) {
  ...
  adj_zip_coverage_info <-
    post_draws_get_prop_coverage_adjusted(
      zip_code_draws, county_positive_prop_info$pos_prop,
  ...
  prop_coverage_info <-
    post_draws_get_prop_coverage(
      inv.logit(zip_code_draws), mrpN_county_positive_prop_info$pos_prop)
  mrp_zip_coverage_m[i,] <- prop_coverage_info[[1]]
  mrp_zip_coverage_length_m[i,] <- prop_coverage_info[[2]]
  adj_zip_coverage_info <-
    post_draws_get_prop_coverage_adjusted(
      zip_code_draws, mrpN_county_positive_prop_info$pos_prop,
      aggregate_sim_data(sim_reduced_covid_data))
...
#save(coverage_m, prop_coverage_m, adj_prop_coverage_m, prop_coverage_length_m, adj_prop_coverage_length_m, zip_coverage_m, zip_coverage_length_m, adj_zip_coverage_m, adj_zip_coverage_length_m, bias_m, file = result_file)
#save(new_gen_prop, sim_data_100, new_stan_results,  file = "result_files/spatial_area_sim_results.Rdata")
